function conf  = ExtractAndSaveFeatures_inv3(conf)
    fprintf('\n -----------------------------------------------');
    ...
    elseif strcmp(conf.feature.typeFeature,'gist')
        ...
        if strcmp(conf.datasetName ,'Caltech256')
            if extractKeypoints(conf) ~=0  % 0: error, 1: ready : 2: extracte            
        ...
        elseif strcmp(conf.datasetName ,'ILSVRC2010')  
            ...
            fprintf('\n\t Extracting features...');
            targetset = 'train';
            ilsvrc.ILSVRC_ExtractFeature_inv3(conf, targetset, conf.BOW.pooler); 
            targetset = 'val';
    ...
    function result = calHistogram(conf)
        numClass = length(conf.class.Names);
        if numClass <1            
            result = 0;
            return ;
        ...
        for ci = 1:numClass
            ...
            % ghi data da encode
             ...
             save(filename,  'setOfFeatures','-v7.3'); 
    ...
    % --------------------------------------------------------------------
    % Feature: xy ly song song theo tung thu muc
    function result = extractKeypoints(conf) 
