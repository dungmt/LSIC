function TrainingTesting_All(conf, start_Idx,end_Idx, step)
...
%                     clear pre_testtrain_matrix;                    
    ...
    function [predicted_label, accuracy, decision_values] = MyPredict(solver,label_vector_test, instance_matrix_test, model)
        switch solver
            case 'libsvm'
                [predicted_label, accuracy, decision_values]= svmpredict(label_vector_test, instance_matrix_test, model,'-b 1');
            case 'liblinear'
                [predicted_label, accuracy, decision_values] = predict(label_vector_test, instance_matrix_test, model, '-b 1');   
            case {'sgd', 'sdca'}
                % --------------------------------------------------------------------
                %                                                  Compute feature map
                % --------------------------------------------------------------------
                ...
                predict_tmp = zeros(size(scores));
                predict_tmp(find(scores>=0))= i ;
                accuracy = length(find(predict_tmp==label_vector_test)) %/length(find(val_label_vector==i))           
        end
    end
