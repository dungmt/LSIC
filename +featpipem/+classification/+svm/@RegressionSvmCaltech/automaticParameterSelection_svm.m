function [bestc, bestg, bestcv ] = automaticParameterSelection_svm(trainLabel, trainData, Ncv, option)
...
while abs(deltacv) > epsilon && cnt < Nlimit
    ...
    numLog2g = length(log2g_list);
        ...
        for i = 1:numLog2c
            log2c = log2c_list(i);   % -c cost : set the parameter C
            for j = 1:numLog2g       % -g gamma : set gamma in kernel function (default 1/num_features)
                log2g = log2g_list(j);
    % % %             % With some kernel
    ...
    % % %             cv = get_cv_ac(trainLabel, [(1:NTrain)' trainData*trainData'], cmd, Ncv);
                ...
                cv = get_cv_ac_svm(trainLabel, trainData, cmd, Ncv);
                if (cv >= bestcv),                
