function train_linear(obj, input, labels,liblinear_options)
...
%         libsvm{ci} = svmtrain(training_label_vector, training_instance_matrix, optparam);
      %  liblinear_options = '-s 0 -p 0 -c 0.001';
        
     %   fprintf('\nlinear Learning model %d with option=%s\n',ci,liblinear_options);
     % chon tham so toi uu
        [bestc, bestg, bestcv,bests] = automaticParameterSelection(training_label_vector, training_instance_matrix);
     
         liblinear_options = [' -q -s ', num2str(bests), ' -c ', num2str(bestc), '-p ', num2str(bestg)];   
        libsvm{ci} = train(training_label_vector, training_instance_matrix, liblinear_options);
    end
