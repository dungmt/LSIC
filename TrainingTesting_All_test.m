function TrainingTesting_All_test(conf, start_Idx,end_Idx, step)
...
% 
    ...
    %% ------
    fprintf('\n +----------------------------------------------------+'); 
    fprintf('\n | Training model of binary classifiers 2...                   |');   
    fprintf('\n +----------------------------------------------------+');
...
%         path_filename_model = fullfile(pathToDirTrain,filename_model );   
        ...
        filename_libsvm_train      =     [ ClassName ,conf.val.midle_file_test,'.train.mat'] ;
        path_filename_libsvm_train = fullfile(pathToDirTrain,filename_libsvm_train);
        
        if exist( path_filename_libsvm_train, 'file')            
            fprintf('finish (ready) !'); 
        else
            if strcmp(conf.svm.solver, 'libsvm')
                if (preComputed_Kernel)	   
                    ...
                    numTrain = length(label_vector);
                    input = [(1:numTrain)', pre_matrix+eye(numTrain)*realmin];    
                    if ~isa(input,'double');
                        input =double(input);
                    end                  
                    ...
                    [predicted_label, accuracy, decision_values]= svmpredict(label_vector', sparse(instance_matrix), model,'-b 1');
                end
                
            elseif strcmp(conf.svm.solver, 'liblinear')
                [predicted_label, accuracy, decision_values] = predict(label_vector', sparse(instance_matrix), model, '-b 1');            
            
            end

            fprintf('\n\t\t Saving result: %s...', filename_libsvm_train);
            save(path_filename_libsvm_train, 'predicted_label', 'accuracy', 'decision_values','label_vector','-v7.3');
            fprintf('finish !');
        end
...
%                         fprintf('finish !');
        ...
        %             if ~exist (path_filename_libsvm_test, 'file')
                ...
                elseif strcmp(conf.svm.solver, 'liblinear')
                    [predicted_label, accuracy, decision_values] = predict(testing_label_vector, sparse(testing.instance_matrix'), model, '-b 1');            
