...
% NII
function LSIC5_Cal( dataset, start_Idx,end_Idx, step,ci_start,ci_end,num_Child, isAuto)
...
%   conf.isOverWriteResult = false;
[ conf ] = TestOnTrainOneVsAll( conf , start_Idx,end_Idx, step);
%     [ conf ] = TrainingTesting_All4(conf, start_Idx,end_Idx, step);

CombineResultTest_Train( conf);
...
%     [conf] =  HC.LabelTree( conf,Q,isAuto);
    ...
    conf.isOverWriteSVRTest=true   ;
  
 
   % Thuc hien training
      conf  = pseudoClass.Train_Train(conf,ci_start,ci_end);
     % Thuc hien testing
