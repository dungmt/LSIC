...
%   34: Formulation as classification problem
   ...
   %% ---------------------------------------------------------------------
   [conf] = InitRPQ(conf, optionR, approach);
...
function [conf] = InitRPQ(conf, optionR, approach)
    ...
    filename_RPQPkQk                        =  sprintf('%s.R%dPQPkQk.mat',str_prefix,optionR);
    filename_RPQPkQk_eigen_approach         =  sprintf('%s.R%dPQPkQk.eigs.app%d.mat',str_prefix,optionR,approach);
    filename_RPQPkQk_eigen_approach_results =  sprintf('%s.R%dPQPkQk.eigs.app%d.results.mat',str_prefix,optionR,approach);
    ...
    conf.experiment.RPQ.path_filename_RPQPkQk       = fullfile(pathToRPQTrains,    filename_RPQPkQk)  ;
...
function [conf] = ComputingPQPkQk(conf)
    ...
    if ~exist(conf.experiment.RPQ.path_filename_RPQPkQk_eigen_approach,'file')            
        ...
        if ~exist(conf.experiment.RPQ.path_filename_RPQPkQk,'file')   
            %%------------------------------------------------------------- 
            ...
            fprintf('\n\t Saving filename: %s ....',conf.experiment.RPQ.path_filename_RPQPkQk);
            save(conf.experiment.RPQ.path_filename_RPQPkQk,'P','Q','Pk','Qk','-v7.3');
            fprintf('done');
%         else
...
function conf = JointOptimizationWithHighDimensionApproach(conf)
    if ~exist(conf.experiment.RPQ.path_filename_RPQPkQk_eigen_approach,'file')
        ...
        fprintf('\n Loading RPQ from filename: %s ....',conf.experiment.RPQ.path_filename_RPQPkQk);
        load(conf.experiment.RPQ.path_filename_RPQPkQk);
        fprintf('done');
