function conf=TrainingTesting_All4(conf, start_Idx,end_Idx, step)
...
%     end
    ...
    fprintf('\n\t conf.svm.num_img_pos_per_class = %.2f',conf.svm.num_img_pos_per_class);
    fprintf('\n\t conf.svm.num_img_neg_per_class = %.2f',conf.svm.num_img_neg_per_class);
    fprintf('\n\t conf.svm.solver = %s', conf.svm.solver);
    
%     if num_img_neg_per_class==0
...
%             end
        ...
        if exist( path_filename_libsvm_val, 'file')            
            ...
            [predicted_label, accuracy, decision_values] = MyPredict(conf.svm.solver,val_label_vector_test, sparse(validation.instance_matrix'), model) ;
...
% model
% pause;
                [predicted_label, accuracy, decision_values] = MyPredict(conf.svm.solver,testing_label_vector, sparse(double(setOfFeatures')), model);
    ...
    %%
    % Input: label_vector_test: 1x n_item
